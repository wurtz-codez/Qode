"""
Qode-Enhanced Agent for SWE-bench Evaluation

Extends mini-swe-agent's DefaultAgent with:
1. Native augment mode: Qode tools via eval-server + grep enrichment (recommended)
2. Native mode: Qode tools via eval-server only
3. Baseline mode: Pure mini-swe-agent (no Qode — control group)
...
logger = logging.getLogger("qode_agent")

PROMPTS_DIR = Path(__file__).parent.parent / "prompts"


class QodeMode(str, Enum):
    """Evaluation modes for Qode integration."""

    BASELINE = "baseline"  # No Qode — pure mini-swe-agent
    NATIVE = "native"  # Qode tools via eval-server
    NATIVE_AUGMENT = "native_augment"  # Native tools + grep enrichment (recommended)


class QodeAgentConfig(AgentConfig):
    """Extended config for Qode evaluation agent."""

    qode_mode: QodeMode = QodeMode.BASELINE
    augment_timeout: float = 5.0
    augment_min_pattern_length: int = 3
    track_qode_usage: bool = True


class QodeAgent(DefaultAgent):
    """
    Agent that optionally enriches its capabilities with Qode code intelligence.

    In BASELINE mode, behaves identically to DefaultAgent.
    In NATIVE mode, Qode tools are available as bash commands via eval-server.
    In NATIVE_AUGMENT mode, Qode tools + automatic grep result enrichment.
    """
    ...
    def __init__(
        ...
        env: Environment,
        *,
        config_class: type = QodeAgentConfig,
        **kwargs,
    ):
        mode = kwargs.get("qode_mode", QodeMode.BASELINE)
        ...
        self.qode_mode = mode
        self.qode_metrics = QodeMetrics()

    def execute_actions(self, message: dict) -> list[dict]:
        """Execute actions with optional Qode augmentation and tracking."""
        if self.config.track_qode_usage:
        ...
        if self.qode_mode == QodeMode.NATIVE_AUGMENT:
    ...
    def _maybe_augment(self, action: dict, output: dict) -> dict | None:
        ...
        with Qode knowledge graph context.
        """
        ...
        try:
            augment_result = self.env.execute(
                {
                    "command": f'qode-augment "{pattern}" 2>&1 || true',
                    "timeout": self.config.augment_timeout,
                }
            )
            elapsed = time.time() - start
            self.qode_metrics.augmentation_calls += 1
            self.qode_metrics.augmentation_time += elapsed

            augment_text = augment_result.get("output", "").strip()
            if augment_text and "[Qode]" in augment_text:
                original_output = output.get("output", "")
                output = dict(output)
                output["output"] = f"{original_output}\n\n{augment_text}"
                self.qode_metrics.augmentation_hits += 1
                return output
        except Exception as e:
            logger.debug(f"Augmentation failed for pattern '{pattern}': {e}")
            self.qode_metrics.augmentation_errors += 1
    ...
    def _track_tool_usage(self, message: dict):
        """Track which Qode tools the agent uses."""
        for action in message.get("extra", {}).get("actions", []):
            command = action.get("command", "")
            if "qode-query" in command:
                self.qode_metrics.tool_calls["query"] += 1
            elif "qode-context" in command:
                self.qode_metrics.tool_calls["context"] += 1
            elif "qode-impact" in command:
                self.qode_metrics.tool_calls["impact"] += 1
            elif "qode-cypher" in command:
                self.qode_metrics.tool_calls["cypher"] += 1
            elif "qode-overview" in command:
                self.qode_metrics.tool_calls["overview"] += 1

    def serialize(self, *extra_dicts) -> dict:
        """Serialize with Qode-specific metrics."""
        qode_data = {
            "info": {
                "qode": {
                    "mode": self.qode_mode.value,
                    "metrics": self.qode_metrics.to_dict(),
                },
            },
        }
        return super().serialize(qode_data, *extra_dicts)


class QodeMetrics:
    """Tracks Qode-specific metrics during evaluation."""
