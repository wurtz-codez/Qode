#!/usr/bin/env python3
"""
Qode SWE-bench Evaluation Runner

Main entry point for running SWE-bench evaluations with and without Qode.
Supports running a single configuration or a full matrix of models x modes.

Usage:
    # Single run (default: native_augment mode — Qode tools + grep enrichment)
...
logger = logging.getLogger("qode_eval")
console = Console()
...
def load_instances(
    subset: str, split: str, slice_spec: str = "", filter_spec: str = ""
) -> list[dict]:
...
def process_instance(
    ...
    try:
        ...
        if env_class_name == "eval.environments.qode_docker.QodeDockerEnvironment":
            from environments.qode_docker import QodeDockerEnvironment

            env_config["image"] = get_swebench_docker_image(instance)
            env = QodeDockerEnvironment(**env_config)
        else:
            from minisweagent.environments.docker import DockerEnvironment

            env = DockerEnvironment(
                image=get_swebench_docker_image(instance), **env_config
            )

        # Build agent
        agent_config = dict(config.get("agent", {}))
        agent_class_name = agent_config.pop(
            "agent_class", "eval.agents.qode_agent.QodeAgent"
        )

        from agents.qode_agent import QodeAgent

        traj_path = instance_dir / f"{instance_id}.traj.json"
        agent_config["output_path"] = traj_path
        agent = QodeAgent(model, env, **agent_config)
        ...
        result["qode_metrics"] = agent.qode_metrics.to_dict()
        ...
        _update_preds(
            output_dir / run_id / "preds.json", instance_id, model_name, result
        )
...
def run_configuration(
    ...
    console.print(
        f"  [bold]{run_id}[/bold]: {len(instances)} instances, {workers} workers"
    )
    ...
    if workers <= 1:
        for instance in instances:
            result = process_instance(
                instance, config, output_dir, model_name, mode_name
            )
    ...
    (
        (run_dir / "summary.json").mkdir(parents=True, exist_ok=True)
...
def single(
    model: str = typer.Option(
        ...
        "-m",
        "--model",
        help=f"Model config name. Available: {', '.join(AVAILABLE_MODELS)}",
    ),
    mode: str = typer.Option(
        "native_augment",
        "--mode",
        help=f"Evaluation mode. Available: {', '.join(AVAILABLE_MODES)}",
    ),
    subset: str = typer.Option(
        "lite", "--subset", help="SWE-bench subset: lite, verified, full"
    ),
    ...
    output: str = typer.Option(
        str(DEFAULT_OUTPUT_DIR), "-o", "--output", help="Output directory"
    ),
...
def matrix(
    models: list[str] = typer.Option(
        ...
        "-m",
        "--models",
        help="Models to evaluate (comma-separated or repeated)",
    ),
    modes: list[str] = typer.Option(
        AVAILABLE_MODES, "--modes", help="Modes to evaluate"
    ),
    ...
    workers: int = typer.Option(
        1, "-w", "--workers", help="Parallel workers per config"
    ),
    output: str = typer.Option(
        str(DEFAULT_OUTPUT_DIR), "-o", "--output", help="Output directory"
    ),
    ...
    for model_name, mode_name in combos:
        ...
        results = run_configuration(
            model_name, mode_name, instances, output_dir, workers, redo
        )
...
def debug(
    model: str = typer.Option(
        "claude-haiku", "-m", "--model", help="Model config name"
    ),
    mode: str = typer.Option("native_augment", "--mode", help="Evaluation mode"),
    instance_id: str = typer.Option(
        ..., "-i", "--instance", help="SWE-bench instance ID"
    ),
    subset: str = typer.Option("lite", "--subset", help="SWE-bench subset"),
    ...
    dataset_path = DATASET_MAPPING.get(subset, subset)
    instances = {
        inst["instance_id"]: inst for inst in load_dataset(dataset_path, split=split)
    }

    if instance_id not in instances:
        console.print(
            f"[red]Instance '{instance_id}' not found in {subset}/{split}[/red]"
        )
        raise typer.Exit(1)
...
def list_configs():
    ...
    for name in AVAILABLE_MODES:
        ...
        gn_mode = config.get("agent", {}).get("qode_mode", "baseline")
        console.print(f"  {name:<20} qode_mode={gn_mode}")
...
def _print_summary(results: list[dict], model: str, mode: str):
    ...
    # Qode-specific metrics
    gn_tool_calls = sum(
        r.get("qode_metrics", {}).get("total_tool_calls", 0) for r in results
    )
    gn_augment_hits = sum(
        r.get("qode_metrics", {}).get("augmentation_hits", 0) for r in results
    )
    if gn_tool_calls > 0:
        table.add_row("Qode Tool Calls", str(gn_tool_calls))
