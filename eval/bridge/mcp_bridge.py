...
Starts the Qode MCP server as a subprocess and provides a Python interface
...
class MCPBridge:
    """
    Manages a Qode MCP server subprocess and proxies tool calls to it.
    ...
    def start(self) -> bool:
        """Start the Qode MCP server subprocess."""
        if self._started:
            return True

        try:
            # Find qode binary
            qode_bin = self._find_qode()
            if not qode_bin:
                logger.error("Qode not found. Install with: npm install -g qode")
                return False
            ...
            init_result = self._send_request(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "qode-eval", "version": "0.1.0"},
                },
            )
    ...
    def call_tool(
        ...
        """
        Call a Qode MCP tool and return the result.
    ...
    def _find_qode(self) -> str | None:
        """Find the qode CLI binary."""
        ...
        for cmd in ["npx"]:
            try:
                ...
                if result.returncode == 0:
                    return cmd  # Will use "npx qode mcp"
            except Exception:
                continue
...
class MCPToolCLI:
    ...
    def run(self, tool_name: str, args_json: str = "{}") -> int:
        ...
        if not self.bridge.start():
            print("ERROR: Failed to start Qode MCP bridge", file=sys.stderr)
            return 1
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python -m bridge.mcp_bridge <tool_name> [args_json]",
            file=sys.stderr,
        )
        print(
            "Tools: query, context, impact, cypher, list_repos, detect_changes, rename",
            file=sys.stderr,
        )
        sys.exit(1)
