#!/usr/bin/env python3
"""
Results Analyzer for Qode SWE-bench Evaluation
...
def compute_metrics(run_data: dict) -> dict:
    ...
    return {
        ...
        "avg_gn_tool_calls": (
            sum(gn_tool_calls) / max(len(gn_tool_calls), 1) if gn_tool_calls else 0
        ),
        "total_augment_hits": sum(gn_augment_hits),
        "total_augment_calls": sum(gn_augment_calls),
        "augment_hit_rate": (
            sum(gn_augment_hits) / max(sum(gn_augment_calls), 1)
            if gn_augment_calls
            else 0
        ),
    }
...
def run_swebench_evaluation(
    results_dir: Path, run_id: str, subset: str = "lite"
) -> dict | None:
    """
...
def summary(
    ...
    format: str = typer.Option(
        "table", "--format", help="Output format: table, markdown, json, csv"
    ),
    swebench_eval: bool = typer.Option(
        False, "--swebench-eval", help="Run official SWE-bench test evaluation"
    ),
    subset: str = typer.Option(
        "lite", "--subset", help="SWE-bench subset (for --swebench-eval)"
    ),
):
    ...
    for run_id, run_data in runs.items():
        ...
        if swebench_eval:
            ...
            if eval_result:
                ...
                metrics["resolve_rate"] = eval_result.get("resolved", 0) / max(
...
def compare_modes(
    ...
    model: str = typer.Option(
        ..., "-m", "--model", help="Model to compare across modes"
    ),
):
...
def qode_usage(
    ...
    """Analyze Qode tool usage patterns across all runs."""
    results_path = Path(results_dir)
    runs = load_run_results(results_path)

    console.print("\n[bold]Qode Tool Usage Analysis[/bold]\n")
    ...
    for run_id, run_data in sorted(runs.items()):
        ...
        for traj in run_data.get("trajectories", {}).values():
            gn = traj.get("info", {}).get("qode", {}).get("metrics", {})
