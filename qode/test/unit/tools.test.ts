 ...
 *
 * Tests: QODE_TOOLS from tools.ts
...
import { QODE_TOOLS, type ToolDefinition } from '../../src/mcp/tools.js';

describe('QODE_TOOLS', () => {
  it('exports exactly 7 tools', () => {
    expect(QODE_TOOLS).toHaveLength(7);
  });

  it('contains all expected tool names', () => {
    const names = QODE_TOOLS.map(t => t.name);
  ...
  it('query tool requires "query" parameter', () => {
    const queryTool = QODE_TOOLS.find(t => t.name === 'query')!;
