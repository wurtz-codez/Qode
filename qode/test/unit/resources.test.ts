...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find(d => d.uri === 'qode://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find(d => d.uri === 'qode://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes context, clusters, processes, schema, cluster detail, process detail', () => {
    ...
    expect(uris).toContain('qode://repo/{name}/context');
    expect(uris).toContain('qode://repo/{name}/clusters');
    expect(uris).toContain('qode://repo/{name}/processes');
    expect(uris).toContain('qode://repo/{name}/schema');
    expect(uris).toContain('qode://repo/{name}/cluster/{clusterName}');
    expect(uris).toContain('qode://repo/{name}/process/{processName}');
  });
...
describe('readResource', () => {
  it('routes qode://repos to listRepos', async () => {
    ...
    const result = await readResource('qode://repos', backend);
  ...
  it('routes qode://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        { name: 'proj', path: '/tmp/proj', indexedAt: '2024-01-01', lastCommit: 'abc', stats: { nodes: 10, edges: 20, processes: 3 } },
      ],
    });
    const result = await readResource('qode://setup', backend);
    expect(result).toContain('Qode MCP');
  ...
  it('routes qode://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('qode://repo/test-project/context', backend);
  ...
  it('routes qode://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('qode://repo/any/schema', backend);
    expect(result).toContain('Qode Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes qode://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
        clusters: [
    ...
    const result = await readResource('qode://repo/test/clusters', backend);
  ...
  it('routes qode://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [
    ...
    const result = await readResource('qode://repo/test/processes', backend);
  ...
  it('routes qode://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('qode://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('qode://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes qode://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('qode://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('qode://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('qode://unknown', backend))
      .rejects.toThrow('Unknown resource URI');
  });

  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('qode://repo/test/nonexistent', backend))
      .rejects.toThrow('Unknown resource');
  });

  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('qode://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('qode://repo/test/cluster/Auth%20Module', backend);
