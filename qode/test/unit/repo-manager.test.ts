...
describe('getStoragePath', () => {
  it('appends .qode to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.qode');
    expect(path.basename(result)).toBe('.qode');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, kuzuPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.qode');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.qode/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('qode-config-test-');
