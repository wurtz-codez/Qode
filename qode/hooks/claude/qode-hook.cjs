...
function main() {
  try {
    ...
    // 2. require.resolve (works when qode is globally installed)
    // 3. Fall back to npx (works when neither is available)
    let cliPath = path.resolve(__dirname, '..', '..', 'dist', 'cli', 'index.js');
    if (!fs.existsSync(cliPath)) {
      try {
        cliPath = require.resolve('qode/dist/cli/index.js');
      } catch {
    ...
    console.error('Qode hook error:', err.message);
  }
}
