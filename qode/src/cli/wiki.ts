...
export const wikiCommand = async (
  ...
  if (!meta) {
    console.log('  Error: No Qode index found.');
    console.log('  Run `qode analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (options?.apiKey || options?.model || options?.baseUrl) {
    ...
    console.log('  Config saved to ~/.qode/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      if (!llmConfig.apiKey) {
        ...
        console.log('  Set OPENAI_API_KEY or QODE_API_KEY environment variable,');
      ...
      const envKey = process.env.QODE_API_KEY || process.env.OPENAI_API_KEY || '';
      let key: string;
      if (envKey) {
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `qode wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync('gh', [
      ...
      '--desc', 'Repository Wiki — generated by Qode',
      '--public',
