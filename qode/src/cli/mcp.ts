...
export const mcpCommand = async () => {
  ...
  if (repos.length === 0) {
    console.error('Qode: No indexed repos yet. Run `qode analyze` in a git repo — the server will pick it up automatically.');
  } else {
    console.error(`Qode: MCP server starting with ${repos.length} repo(s): ${repos.map(r => r.name).join(', ')}`);
  }
