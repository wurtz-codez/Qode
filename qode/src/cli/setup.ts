 ...
 * so the Qode MCP server is available in all projects.
 */
...
/**
 * Merge qode entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.qode = getMcpEntry();
  return existing;
}
...
async function setupClaudeCode(result: SetupResult): Promise<void> {
  ...
  console.log('');
  console.log('  Claude Code detected. Run this command to add Qode MCP:');
  console.log('');
  console.log('    claude mcp add qode -- npx -y qode mcp');
  console.log('');
  result.configured.push('Claude Code (MCP manual step printed)');
}

/**
 * Install Qode skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install Qode hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the qode package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/qode/
  const destHooksDir = path.join(claudeDir, 'hooks', 'qode');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'qode-hook.cjs');
    const dest = path.join(destHooksDir, 'qode-hook.cjs');
    try {
    ...
    const hookCmd = `node "${path.join(destHooksDir, 'qode-hook.cjs').replace(/\\/g, '/')}"`;
    ...
    const hasPreToolHook = existing.hooks.PreToolUse.some(
      (h: any) => h.hooks?.some((hh: any) => hh.command?.includes('qode'))
    );
    if (!hasPreToolHook) {
      existing.hooks.PreToolUse.push({
        ...
        hooks: [{
          ...
          command: hookCmd,
          timeout: 8000,
          statusMessage: 'Enriching with Qode graph context...',
        }],
      });
    }
...
const SKILL_NAMES = ['qode-exploring', 'qode-debugging', 'qode-impact-analysis', 'qode-refactoring', 'qode-guide', 'qode-cli'];

/**
 * Install Qode skills to a target directory.
 * Each skill is installed as {targetDir}/qode-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/qode/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/qode/
 */
...
export const setupCommand = async () => {
  ...
  console.log('    2. Run: qode analyze');
