...
import { QODE_TOOLS } from './tools.js';
...
function getNextStepHint(toolName: string, args: Record<string, any> | undefined): string {
  ...
  switch (toolName) {
    case 'list_repos':
      return `\n\n---\n**Next:** READ qode://repo/{name}/context for any repo above to get its overview and check staleness.`;
    ...
    case 'context':
      return `\n\n---\n**Next:** If planning changes, use impact({target: "${args?.name || '<name>'}", direction: "upstream"${repoParam}}) to check blast radius. To see execution flows, READ qode://repo/${repoPath}/processes.`;

    case 'impact':
      return `\n\n---\n**Next:** Review d=1 items first (WILL BREAK). To check affected execution flows, READ qode://repo/${repoPath}/processes.`;

    case 'detect_changes':
      return `\n\n---\n**Next:** Review affected processes. Use context() on high-risk changed symbols. READ qode://repo/${repoPath}/process/{name} for full execution traces.`;

    case 'rename':
      return `\n\n---\n**Next:** Run detect_changes(${repoParam ? `{repo: "${repo}"}` : ''}) to verify no unexpected side effects from the rename.`;

    case 'cypher':
      return `\n\n---\n**Next:** To explore a result symbol, use context({name: "<name>"${repoParam}}). For schema reference, READ qode://repo/${repoPath}/schema.`;
    ...
    case 'overview':
      return `\n\n---\n**Next:** To drill into an area, READ qode://repo/${repoPath}/cluster/{name}. To see execution flows, READ qode://repo/${repoPath}/processes.`;
...
Follow these steps:
1. READ \`qode://repo/${repo || '{name}'}/context\` for codebase stats
2. READ \`qode://repo/${repo || '{name}'}/clusters\` to see all functional areas
3. READ \`qode://repo/${repo || '{name}'}/processes\` to see all execution flows
4. For the top 5 most important processes, READ \`qode://repo/${repo || '{name}'}/process/{name}\` for step-by-step traces
