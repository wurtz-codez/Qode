 ...
 *
 * Provides tool implementations using local .qode/ indexes.
...
import { initKuzu, executeQuery, executeParameterized, closeKuzu, isKuzuReady } from '../core/kuzu/kuzu-adapter.js';
...
// AI context generation is CLI-only (qode analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`Qode [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: qode analyze');
    }
    if (repoParam) {
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('Qode: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
  ...
  private async semanticSearch(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    try {
      ...
      const { embedQuery, getEmbeddingDims } = await import('../core/embeddings/embedder.js');
