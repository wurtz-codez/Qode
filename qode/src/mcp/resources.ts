 ...
 * All resources use repo-scoped URIs: qode://repo/{name}/context
 */
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'qode://repos') return { resourceType: 'repos' };
  if (uri === 'qode://setup') return { resourceType: 'setup' };

  // Repo-scoped: qode://repo/{name}/context
  const repoMatch = uri.match(/^qode:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: qode analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# qode_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: qode analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx qode analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - qode://repos: All indexed repositories');
  lines.push(`  - qode://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - qode://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - qode://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - qode://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: qode analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use qode_query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: qode analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use qode_query for deeper search.`);
    }
 ...
 * Useful for `qode setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# Qode\n\nNo repositories indexed. Run: `npx qode analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# Qode MCP — ${repo.name}`,
      '',
      `This project is indexed by Qode as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`qode://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`qode://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`qode://repo/${repo.name}/processes\` — All execution flows`,
      `- \`qode://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
