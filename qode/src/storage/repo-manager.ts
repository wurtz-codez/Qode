 ...
 * Repository Manager
 *
 * Manages Qode index storage in .qode/ at repo root.
 * Also maintains a global registry at ~/.qode/registry.json
...
/**
 * Shape of an entry in the global registry (~/.qode/registry.json)
 */
...
/**
 * Get the .qode storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), QODE_DIR);
};
...
/**
 * Check if a path has a Qode index
 */
...
/**
 * Find .qode by walking up from a starting path
 */
...
/**
 * Add .qode to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(QODE_DIR)) return;
    ...
    await fs.writeFile(gitignorePath, `${QODE_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.qode/registry.json) ───────────────────────

/**
 * Get the path to the global Qode directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.qode');
};
 ...
 * Called after `qode analyze` completes.
 */
...
// ─── Global CLI Config (~/.qode/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.qode/config.json
 */
...
/**
 * Save CLI config to ~/.qode/config.json
 */
