 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .qode/ index.
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: qode analyze' });
        return;
      }
  ...
  const server = app.listen(port, host, () => {
    console.log(`Qode server running on http://${host}:${port}`);
  });
